...
// CipherMutation represents an operation that mutates the Cipher nodes in the graph.
type CipherMutation struct {
	config
	op            Op
	typ           string
	id            *int
	...
	oldValue      func(context.Context) (*Cipher, error)
	predicates    []predicate.Cipher
}

var _ ent.Mutation = (*CipherMutation)(nil)

// cipherOption allows management of the mutation configuration using functional options.
type cipherOption func(*CipherMutation)

// newCipherMutation creates new mutation for the Cipher entity.
func newCipherMutation(c config, op Op, opts ...cipherOption) *CipherMutation {
	m := &CipherMutation{
		config:        c,
		op:            op,
		typ:           TypeCipher,
...
// withCipherID sets the ID field of the mutation.
func withCipherID(id int) cipherOption {
	return func(m *CipherMutation) {
		var (
			err   error
			once  sync.Once
			value *Cipher
		)
		m.oldValue = func(ctx context.Context) (*Cipher, error) {
			once.Do(func() {
				if m.done {
					err = fmt.Errorf("querying old values post mutation is not allowed")
				} else {
					value, err = m.Client().Cipher.Get(ctx, id)
				}
			})
			return value, err
		}
...
// withCipher sets the old Cipher of the mutation.
func withCipher(node *Cipher) cipherOption {
	return func(m *CipherMutation) {
		m.oldValue = func(context.Context) (*Cipher, error) {
			return node, nil
		}
		m.id = &node.ID
	}
...
// Client returns a new `ent.Client` from the mutation. If the mutation was
// executed in a transaction (ent.Tx), a transactional client is returned.
func (m CipherMutation) Client() *Client {
	client := &Client{config: m.config}
	client.init()
	return client
}
...
// Tx returns an `ent.Tx` for mutations that were executed in transactions;
// it returns an error otherwise.
func (m CipherMutation) Tx() (*Tx, error) {
	if _, ok := m.driver.(*txDriver); !ok {
		return nil, fmt.Errorf("ent: mutation is not running in a transaction")
	}
	tx := &Tx{config: m.config}
...
// ID returns the ID value in the mutation. Note that the ID
// is only available if it was provided to the builder.
func (m *CipherMutation) ID() (id int, exists bool) {
	if m.id == nil {
		return
	}
	return *m.id, true
...
func (m *CipherMutation) SetDefault(b []byte) {
	m._default = &b
}

// Default returns the value of the "default" field in the mutation.
func (m *CipherMutation) Default() (r []byte, exists bool) {
	v := m._default
	if v == nil {
		return
	}
...
// OldDefault returns the old "default" field's value of the Cipher entity.
// If the Cipher object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *CipherMutation) OldDefault(ctx context.Context) (v []byte, err error) {
	if !m.op.Is(OpUpdateOne) {
...
func (m *CipherMutation) ResetDefault() {
	m._default = nil
}

// Op returns the operation name.
func (m *CipherMutation) Op() Op {
	return m.op
}

// Type returns the node type of this mutation (Cipher).
func (m *CipherMutation) Type() string {
	return m.typ
}
...
// order to get all numeric fields that were incremented/decremented, call
// AddedFields().
func (m *CipherMutation) Fields() []string {
	fields := make([]string, 0, 1)
	if m._default != nil {
		fields = append(fields, cipher.FieldDefault)
	}
	return fields
}
...
// return value indicates that this field was not set, or was not defined in the
// schema.
func (m *CipherMutation) Field(name string) (ent.Value, bool) {
	switch name {
...
// returned if the mutation operation is not UpdateOne, or the query to the
// database failed.
func (m *CipherMutation) OldField(ctx context.Context, name string) (ent.Value, error) {
	switch name {
	case cipher.FieldDefault:
		return m.OldDefault(ctx)
	}
	return nil, fmt.Errorf("unknown Cipher field %s", name)
}

// SetField sets the value of a field with the given name. It returns an error if
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *CipherMutation) SetField(name string, value ent.Value) error {
	switch name {
	case cipher.FieldDefault:
		v, ok := value.([]byte)
		if !ok {
	...
	return fmt.Errorf("unknown Cipher field %s", name)
}

// AddedFields returns all numeric fields that were incremented/decremented during
// this mutation.
func (m *CipherMutation) AddedFields() []string {
	return nil
}
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *CipherMutation) AddedField(name string) (ent.Value, bool) {
	return nil, false
}
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *CipherMutation) AddField(name string, value ent.Value) error {
	switch name {
	}
	return fmt.Errorf("unknown Cipher numeric field %s", name)
}

// ClearedFields returns all nullable fields that were cleared during this
// mutation.
func (m *CipherMutation) ClearedFields() []string {
	return nil
}

// FieldCleared returns a boolean indicating if a field with the given name was
// cleared in this mutation.
func (m *CipherMutation) FieldCleared(name string) bool {
	_, ok := m.clearedFields[name]
	return ok
}
...
// ClearField clears the value of the field with the given name. It returns an
// error if the field is not defined in the schema.
func (m *CipherMutation) ClearField(name string) error {
	return fmt.Errorf("unknown Cipher nullable field %s", name)
}

// ResetField resets all changes in the mutation for the field with the given name.
// It returns an error if the field is not defined in the schema.
func (m *CipherMutation) ResetField(name string) error {
	switch name {
	case cipher.FieldDefault:
		m.ResetDefault()
		return nil
...
// AddedEdges returns all edge names that were set/added in this mutation.
func (m *CipherMutation) AddedEdges() []string {
...
// AddedIDs returns all IDs (to other nodes) that were added for the given edge
// name in this mutation.
func (m *CipherMutation) AddedIDs(name string) []ent.Value {
	return nil
}

// RemovedEdges returns all edge names that were removed in this mutation.
func (m *CipherMutation) RemovedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// RemovedIDs returns all IDs (to other nodes) that were removed for the edge with
// the given name in this mutation.
func (m *CipherMutation) RemovedIDs(name string) []ent.Value {
	return nil
}

// ClearedEdges returns all edge names that were cleared in this mutation.
func (m *CipherMutation) ClearedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// EdgeCleared returns a boolean which indicates if the edge with the given name
// was cleared in this mutation.
func (m *CipherMutation) EdgeCleared(name string) bool {
	return false
}

// ClearEdge clears the value of the edge with the given name. It returns an error
// if that edge is not defined in the schema.
func (m *CipherMutation) ClearEdge(name string) error {
	return fmt.Errorf("unknown Cipher unique edge %s", name)
}

// ResetEdge resets all changes to the edge with the given name in this mutation.
// It returns an error if the edge is not defined in the schema.
func (m *CipherMutation) ResetEdge(name string) error {
	return fmt.Errorf("unknown Cipher edge %s", name)
}
